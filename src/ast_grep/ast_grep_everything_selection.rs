//ast_grep_everything_selection.rs
...
fn extract_impls(
    ...
    let impl_body = single_map.get("IMPL_BODY")?;
    Some(vec![ImplSelection {
        file: file.to_string(),
        impl_range: impl_body.range.clone(),
        impl_text: impl_body.text.clone(), // ← added
    }])
}
...
fn extract_methods(
    ...
    let (body, name) = (
        single_map.get("METHOD_BODY")?,
        single_map.get("METHOD_NAME")?,
    );
    let (impl_range, impl_text) = single_map
        .get("METHOD_IMPL_BODY")
        .map(|i| (i.range.clone(), i.text.clone()))
        .unwrap_or_else(|| (body.range.clone(), String::new()));
    Some(vec![MethodSelection {
        file: file.to_string(),
        impl_range,
        impl_text, // ← added
...
fn extract_enums(
    ...
    let (body, name) = (single_map.get("ENUM_BODY")?, single_map.get("ENUM_NAME")?);
    Some(vec![EnumSelection {
        file: file.to_string(),
...
fn extract_unions(
    ...
    let (body, name) = (single_map.get("UNION_BODY")?, single_map.get("UNION_NAME")?);
    Some(vec![UnionSelection {
        file: file.to_string(),
...
fn extract_trait_methods(
    ...
    let method_body = single_map.get("TRAIT_METHOD_BODY")?;
    let method_name = single_map.get("TRAIT_METHOD_NAME")?;

    let trait_body_range = single_map
        .get("TRAIT_BODY_WITH_METHOD")
        .map(|tb| tb.range.clone())
        .unwrap_or_else(|| method_body.range.clone());

    let trait_name_range = single_map
        .get("TRAIT_NAME_WITH_METHOD")
        .map(|tn| tn.range.clone())
        .unwrap_or_else(|| method_name.range.clone());

    Some(vec![TraitMethodDefinitionSelection {
        file: file.to_string(),
        trait_body_range,
...
fn extract_traits(
    ...
    let (body, name) = (single_map.get("TRAIT_BODY")?, single_map.get("TRAIT_NAME")?);
    Some(vec![TraitSelection {
        file: file.to_string(),
...
fn extract_trait_method_signatures(
    ...
    Some(vec![TraitMethodSignatureSelection {
        file: file.to_string(),
...
fn extract_attributes(
    ...
    let attr = single_map.get("ATTRIBUTES")?;
    Some(vec![AttributeSelection {
        file: file.to_string(),
...
fn extract_tests_mods(
    ...
    let tmod = single_map.get("TESTS_MOD")?;
    Some(vec![TestsModSelection {
        file: file.to_string(),
...
fn extract_structs(
    ...
    let (body, name) = (
        single_map.get("STRUCT_BODY")?,
        single_map.get("STRUCT_NAME")?,
    );
    Some(vec![StructSelection {
        file: file.to_string(),
        body_range: body.range.clone(),
        name_range: name.range.clone(),
    }])
...
fn extract_mods(
    ...
    let (body, name) = (single_map.get("MOD_BODY")?, single_map.get("MOD_NAME")?);
    Some(vec![ModSelection {
        file: file.to_string(),
...
fn extract_expression_statements(
    ...
    let expr = single_map.get("EXPRESSION_STATEMENT")?;
    Some(vec![ExpressionStatementSelection {
        file: file.to_string(),
...
fn extract_use_declarations(
    ...
    let use_decl = single_map.get("USE_DECLARATION")?;
    Some(vec![UseDeclarationSelection {
        file: file.to_string(),
...
fn extract_macro_definitions(
    ...
    Some(vec![MacroDefinitionSelection {
        file: file.to_string(),
...
fn extract_macro_invocations(
    ...
    let inv = single_map.get("MACRO_INVOCATION")?;
    Some(vec![MacroInvocationSelection {
        file: file.to_string(),
