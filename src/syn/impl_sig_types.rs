...
impl TypeIdentifiers {
    pub fn from_impl_parts(
        ...
        let (ak_type_variables, ak_concrete_types) = Self::extract_type_identifiers_from_as_keyword(
            (ctx.type_identifiers, ctx.primitive_types),
            as_keyword.as_ref(),
        );
        ...
        (
            TypeIdentifiers {
                type_variables: if type_variables.is_empty() {
                    None
                } else {
                    Some(type_variables)
                },
                concrete_types: if concrete_types.is_empty() {
                    None
                } else {
                    Some(concrete_types)
                },
...
impl TypeIdentifiers {
    fn extract_type_identifiers_from_as_keyword(
        ...
        let type_identifiers_flat: Vec<&NodeMatch> = ctx.0.into_iter().flatten().collect();
        let primitive_types_flat: Vec<&NodeMatch> = ctx.1.into_iter().flatten().collect();
        let mut type_variables: TypeVariableMap = TypeVariableMap::new();
        let mut concrete_types: CTypeSet = CTypeSet::new();
...
impl TypeIdentifiers {
    fn extract_type_identifiers_from_const(
        ...
        let (type_identifiers, primitive_types) = ctx;
        let identifiers_flat: Vec<NodeMatch> = identifiers.into_iter().flatten().cloned().collect();
        let type_identifiers_flat: Vec<NodeMatch> =
            ...
            let mut candidates: Vec<(TypeVariable, usize)> = type_variables
                ...
                .map(|(k, v)| {
                    let unresolved_tv_refs = v
                        .iter()
                        .filter(|t| all_var_names.contains(*t) && !expanded.contains(*t) && *t != k)
                        .count();
...
mod tests {
    ...
    #[test]
    fn test_const_param_primitive_type_with_mixed_generic_params() {
        let const_params = vec![make_node("const SHALLOW: bool", 35, 54)];
        let identifiers = vec![
            make_node("a", 6, 7),
            make_node("tcx", 10, 13),
            make_node("SHALLOW", 41, 48),
            make_node("tcx", 70, 73),
            make_node("a", 92, 93),
            make_node("tcx", 96, 99),
        ];
        let type_identifiers = vec![
            make_node("VL", 15, 17),
            make_node("VisibilityLike", 19, 33),
            make_node("DefIdVisitor", 56, 68),
            make_node("FindMin", 83, 90),
            make_node("VL", 101, 103),
            make_node("SHALLOW", 105, 112),
        ];
        let primitive_types = vec![make_node("bool", 50, 54)];

        let (type_variables, concrete_types) = TypeIdentifiers::extract_type_identifiers_from_const(
            (Some(&type_identifiers), Some(&primitive_types)),
            Some(&const_params),
            Some(&identifiers),
        );
        ...
        let expected_types: TypeSet = ["bool".to_string()].into();
        assert_eq!(type_variables.get("SHALLOW"), Some(&expected_types));
        assert_eq!(type_variables.len(), 1);
        assert_eq!(concrete_types, ["bool".to_string()].into());
    }

    /// impl<'a, T: Clone, const N: usize> Array<T, N>
    /// where T: std::fmt::Debug + Deserialize,
    #[test]
    fn test_const_param_usize_with_where_clause_bounds() {
        let const_params = vec![make_node("const N: usize", 19, 33)];
        let identifiers = vec![
            make_node("a", 6, 7),
            make_node("N", 25, 26),
            make_node("std", 60, 63),
        ...
        let type_identifiers = vec![
            make_node("T", 9, 10),
            make_node("Clone", 12, 17),
            make_node("Array", 35, 40),
            make_node("T", 41, 42),
            make_node("N", 44, 45),
            make_node("T", 57, 58),
            make_node("Debug", 70, 75),
            make_node("Deserialize", 78, 89),
        ];
        let primitive_types = vec![make_node("usize", 28, 33)];
        ...
        let expected_types: TypeSet = ["usize".to_string()].into();
        assert_eq!(type_variables.get("N"), Some(&expected_types));
        assert_eq!(type_variables.len(), 1);
        assert_eq!(concrete_types, ["usize".to_string()].into());
    }

    /// impl<'tcx, C: QueryCache, const FLAGS: QueryFlags> QueryDispatcher<'tcx>
    /// for SemiDynamicQueryDispatcher<'tcx, C, FLAGS>
    #[test]
    fn test_const_param_with_non_primitive_type() {
        let const_params = vec![make_node("const FLAGS: QueryFlags", 26, 49)];
        let identifiers = vec![
            make_node("tcx", 6, 9),
            make_node("FLAGS", 32, 37),
            make_node("tcx", 68, 71),
            make_node("tcx", 109, 112),
        ];
        let type_identifiers = vec![
            make_node("C", 11, 12),
            make_node("QueryCache", 14, 24),
            make_node("QueryFlags", 39, 49),
            make_node("QueryDispatcher", 51, 66),
            make_node("SemiDynamicQueryDispatcher", 81, 107),
            make_node("C", 114, 115),
            make_node("FLAGS", 117, 122),
        ];
        ...
        let expected_types: TypeSet = ["QueryFlags".to_string()].into();
        assert_eq!(type_variables.get("FLAGS"), Some(&expected_types));
        assert_eq!(concrete_types, ["QueryFlags".to_string()].into());
    }

    /// impl<const BR: bool> VecGraph<BR>
    #[test]
    fn test_const_param_with_primitive_type() {
        let const_params = vec![make_node("const BR: bool", 5, 19)];
        let identifiers = vec![make_node("BR", 11, 13)];
        let type_identifiers = vec![make_node("VecGraph", 21, 29), make_node("BR", 30, 32)];
        let primitive_types = vec![make_node("bool", 15, 19)];
        ...
        let expected_types: TypeSet = ["bool".to_string()].into();
        assert_eq!(type_variables.get("BR"), Some(&expected_types));
        assert_eq!(concrete_types, ["bool".to_string()].into());
    }
    ...
    fn test_extract_type_identifiers_from_trait_bounds_deep_reject_ctxt_const_params() {
        ...
        let trait_bounds = vec![make_node(": Interner", 6, 16)];
        ...
        let primitive_types = vec![make_node("bool", 52, 56), make_node("bool", 92, 96)];
    ...
    fn test_extract_type_identifiers_from_trait_bounds_arc_job_fn_send_sync() {
        ...
        let trait_bounds = vec![make_node(": Fn(JobRefId) + Send + Sync", 38, 66)];
    ...
    fn test_extract_type_identifiers_from_trait_bounds_async_write_sink_writer() {
        ...
        let primitive_types = vec![make_node("u64", 35, 38), make_node("u8", 91, 93)];
    ...
    fn test_extract_type_identifiers_from_trait_bounds_canonical_var_values_into_iterator() {
        ...
        let trait_bounds = vec![make_node(": Interner", 10, 20)];
    ...
    fn test_extract_type_identifiers_from_trait_bounds_option_arena_allocatable() {
        ...
        let trait_bounds = vec![make_node(": ArenaAllocatable<'tcx>", 12, 36)];
    ...
    fn test_extract_type_identifiers_from_trait_bounds_table_builder_fixed_size_encoding() {
        ...
        let primitive_types = vec![make_node("usize", 22, 27), make_node("u8", 63, 65)];
    ...
    fn test_extract_type_identifiers_from_trait_bounds_sink_writer_async_write_no_inline_bounds() {
        ...
        let primitive_types = vec![make_node("u8", 71, 73)];
    ...
    fn test_extract_type_identifiers_from_trait_bounds_unord_items_iterator_item() {
        ...
        let trait_bounds = vec![make_node(": Iterator<Item = T>", 9, 29)];
    ...
    fn test_extract_type_identifiers_from_trait_bounds_fingerprint_hasher_for_h() {
        ...
        let trait_bounds = vec![make_node(": Hasher", 6, 14)];
    ...
    fn test_extract_type_identifiers_from_trait_bounds_delegate_for_mut_ref() {
        ...
        let trait_bounds = vec![make_node(": Delegate<'tcx>", 12, 28)];
    ...
    fn test_extract_type_identifiers_from_trait_bounds_rustc_peek_at_analysis() {
        ...
        let trait_bounds = vec![
            ...
            make_node(
                ": JoinSemiLattice + Clone + BitSetExt<MovePathIndex>",
                107,
                159,
            ),
        ];
    ...
    fn test_extract_type_identifiers_from_trait_bounds_move_path_index_has_move_data() {
        ...
        let trait_bounds = vec![make_node(": crate::move_paths::HasMoveData<'tcx>", 82, 120)];
    ...
    fn test_extract_type_identifiers_from_trait_bounds_mixed_bit_set_debug_with_context() {
        ...
        let trait_bounds = vec![make_node(": Idx + DebugWithContext<C>", 61, 88)];
    ...
    fn test_extract_type_identifiers_from_trait_bounds_generic_cx_borrow_scx() {
        ...
        let trait_bounds = vec![make_node(": Borrow<SCx<'ll>>", 12, 30)];
    ...
    fn test_extract_type_identifiers_from_trait_bounds_move_data_builder_fn_ty() {
        ...
        let trait_bounds = vec![make_node(": Fn(Ty<'tcx>) -> bool", 16, 38)];
        ...
        let primitive_types = vec![make_node("bool", 34, 38)];
    ...
    fn test_extract_type_identifiers_from_trait_bounds_basic_blocks_no_bounds() {
        ...
        let type_identifiers = vec![make_node("BasicBlocks", 11, 22)];
    ...
    fn test_extract_type_identifiers_from_trait_bounds_vec_cache_drop_may_dangle() {
        ...
        let trait_bounds = vec![make_node(": Idx", 13, 18)];
