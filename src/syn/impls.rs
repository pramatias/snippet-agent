...
impl AllSynElements {
    ...
    pub fn pick_blanket_impls(&mut self, file_contents: &HashMap<Arc<str>, String>) {
        ...
        let method_index: HashSet<u64> = self
            ...
            .iter()
            .map(|m| {
                hash_pair(&m.file, m.impl_body.text_str()) // no fc needed
            })
            .collect();

        self.syn_impls.retain(|impl_item| {
            !method_index.contains(&hash_pair(&impl_item.file, impl_item.impl_body.text_str()))
        });
    }
}
